...
export function DashboardOverview({ user, sessions, scenarios, isLocked }: DashboardOverviewProps) {
  ...
  if (totalSessions === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] max-w-2xl mx-auto px-4 text-center">
        <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight mb-4">Welcome to Voxa</h1>
        <p className="text-lg text-muted-foreground mb-10">
          Your communication dashboard is empty. Complete your first practice session to unlock your AI coaching report and start building your communication streak.
        </p>
        <a href="/practice" className="px-8 py-4 rounded-full bg-primary text-primary-foreground font-bold text-lg hover:opacity-90 transition-opacity inline-flex items-center gap-2">
          Start First Session
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-arrow-right size-5"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
        </a>
      </div>
    )
  }
